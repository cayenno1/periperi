...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Close sidebar on window resize if going to larger screen
...
let orderFilters = {
    filter: 'new-to-old', // Combined filter for backward compatibility
    sort: 'new-to-old',   // Sort direction: 'new-to-old' or 'old-to-new'
    status: '',           // Status filter: '', 'pending', 'preparing', etc.
    serviceType: '',      // Service type filter: '', 'delivery', 'dine-in', 'pick-up'
    searchTerm: ''
};
...
const MenuStore = (() => {
    ...
    async function deleteItem(id) {
        ...
        const fns = assertFirestoreReady();
        const menuDocRef = fns.doc(window.db, COLLECTION, id);
        const archiveRef = fns.doc(window.db, 'menuArchives', id);
        
        // Check both menu and archive collections
        const menuDoc = await fns.getDoc(menuDocRef);
        const archiveDoc = await fns.getDoc(archiveRef);
        
        // Determine where the item is located
        let currentData = null;
        let itemInMenu = false;
        let itemInArchive = false;
        
        if (menuDoc.exists()) {
            currentData = menuDoc.data();
            itemInMenu = true;
        } else if (archiveDoc.exists()) {
            currentData = archiveDoc.data();
            itemInArchive = true;
        } else {
            throw new Error('Menu item not found.');
        ...
        // Prepare archive data with deletion info
        const archiveData = {
            ...currentData,
            deletedAt: fns.serverTimestamp(),
            archivedAt: currentData.archivedAt || fns.serverTimestamp(),
            archivedFrom: COLLECTION,
        ...
        if (itemInArchive) {
            // Item is already in archive, just update it with deletion info
            await fns.updateDoc(archiveRef, {
                deletedAt: fns.serverTimestamp(),
                deletedByUser: true,
                isDeleted: true,
            ...
            // Item is in menu collection - move to archive
            if (archiveDoc.exists()) {
                // Archive doc already exists (shouldn't happen, but handle it)
                await fns.updateDoc(archiveRef, {
                    ...archiveData,
                    updatedAt: fns.serverTimestamp()
                });
            ...
            await fns.deleteDoc(menuDocRef);
        }
        
        return await getItems();
    }
    
    // Check for expired archived items (older than 30 days) - requires user confirmation to delete
    async function cleanupExpiredArchivedItems() {
        ...
        try {
            ...
            // Check menuArchives collection for expired items
            ...
            // Check menu collection for expired deleted items
            ...
            // Store expired items for later confirmation - do NOT auto-delete
            if (expiredItems.length > 0) {
                console.log(`Found ${expiredItems.length} menu items pending permanent deletion (older than 30 days):`, expiredItems.map(i => i.name));
                // Store for later use - user must manually confirm deletion
                window.pendingPermanentDeletions = expiredItems;
            }
    ...
    // Permanently delete items with user confirmation
    async function permanentlyDeleteExpiredItems(itemsToDelete) {
        const fns = assertFirestoreReady();
        const deletePromises = [];
        
        for (const item of itemsToDelete) {
            deletePromises.push(fns.deleteDoc(fns.doc(window.db, item.collection, item.id)));
        }
        
        if (deletePromises.length > 0) {
            await Promise.all(deletePromises);
            console.log(`Permanently deleted ${deletePromises.length} expired menu items.`);
        }
        
        // Clear pending deletions
        window.pendingPermanentDeletions = [];
...
function normalizeOrderDoc(docSnap) {
    ...
    } else if (['delivered'].includes(statusValue)) {
...
function filterOrdersByCriteria(orders) {
    ...
    // NEW: Apply direct status filter (from new dropdown)
    if (orderFilters.status) {
        const statusFilter = orderFilters.status.toLowerCase();
        filtered = filtered.filter(order => {
    ...
    // NEW: Apply direct service type filter (from new dropdown)
    if (orderFilters.serviceType) {
        const typeFilter = orderFilters.serviceType.toLowerCase();
        filtered = filtered.filter(order => {
    ...
    // LEGACY: Filter by combined filter (status, type, or both) for backward compatibility
    if (!orderFilters.status && !orderFilters.serviceType && orderFilters.filter && orderFilters.filter !== 'all' && orderFilters.filter !== 'old-to-new' && orderFilters.filter !== 'new-to-old') {
...
function renderOrdersTable(orders) {
    ...
    // Sort orders based on filter (use new sort property or fallback to filter)
    let sortedOrders = [...filteredOrders];
    const sortDirection = orderFilters.sort || orderFilters.filter;
    ...
    paginatedOrders.forEach(order => {
        ...
        const isCompleted = orderStatusLower === 'delivered' || orderStatusLower === 'completed';
        ...
        if (isPending) {
            ...
            // From ready for pick-up, can mark as completed
            if (isPickUp) {
                availableStatuses.push({ value: 'completed', label: 'Completed' });
            }
        } else if (orderStatusLower === 'ready') {
            // From ready (dine-in), can mark as completed
            if (isDineIn) {
                availableStatuses.push({ value: 'completed', label: 'Completed' });
            }
            ...
            // From out for delivery, can mark as completed
            if (isDeliveryOrder) {
                availableStatuses.push({ value: 'completed', label: 'Completed' });
            }
        }
        ...
        if (availableStatuses.length > 0 && !isCompleted && !isCancelled) {
        ...
        // Build meatball menu items
        const menuItems = [];
        
        // Always show View Details
        if (order.id) {
            const escapedOrderId = String(order.id).replace(/'/g, "\\'").replace(/"/g, '\\"');
            menuItems.push(`<button class="meatball-menu-item" onclick="event.stopPropagation(); closeOrderMeatballMenu('${escapedOrderId}'); viewOrderDetails('${escapedOrderId}')">
                <i class="fas fa-eye"></i> View Details
            </button>`);
        }
        
        // Add Verify Payment button for unverified GCash orders
        if (order.id && isGCashOrder && !isPaymentVerified) {
            const escapedOrderId = String(order.id).replace(/'/g, "\\'").replace(/"/g, '\\"');
            menuItems.push(`<button class="meatball-menu-item" onclick="event.stopPropagation(); closeOrderMeatballMenu('${escapedOrderId}'); verifyPayment('${escapedOrderId}')">
                <i class="fas fa-receipt"></i> Verify Payment
            </button>`);
        }
        
        // Always show Decline Order button
        if (order.id) {
            const escapedOrderId = String(order.id).replace(/'/g, "\\'").replace(/"/g, '\\"');
            menuItems.push(`<button class="meatball-menu-item" onclick="event.stopPropagation(); closeOrderMeatballMenu('${escapedOrderId}'); declinePayment('${escapedOrderId}')">
                <i class="fas fa-times-circle"></i> Decline Order
            </button>`);
        }
        
        // Add status change options if available
        if (availableStatuses.length > 0 && !isCompleted && !isCancelled && order.id) {
            const canChangeStatus = !isPending || !isGCashOrder || isPaymentVerified;
            if (canChangeStatus) {
                const escapedOrderId = String(order.id).replace(/'/g, "\\'").replace(/"/g, '\\"');
                availableStatuses.forEach(status => {
                    const escapedStatusValue = String(status.value).replace(/'/g, "\\'").replace(/"/g, '\\"');
                    menuItems.push(`<button class="meatball-menu-item" onclick="event.stopPropagation(); closeOrderMeatballMenu('${escapedOrderId}'); updateOrderStatus('${escapedOrderId}', '${escapedStatusValue}')">
                        <i class="fas fa-sync-alt"></i> Change Status: ${status.label}
                    </button>`);
                });
            }
        }
        ...
        if (isDeliveryOrder && (isReadyForDelivery || isOutForDelivery || isPreparing) && order.id) {
            const hasDriver = !!(order.driverId && order.driverId.trim());
            if (!hasDriver) {
                const escapedOrderId = String(order.id).replace(/'/g, "\\'").replace(/"/g, '\\"');
                menuItems.push(`<button class="meatball-menu-item" onclick="event.stopPropagation(); closeOrderMeatballMenu('${escapedOrderId}'); openDriverSelectionForOrder('${escapedOrderId}')">
                    <i class="fas fa-user-plus"></i> Assign Driver
                </button>`);
            }
        }
        
        // Build meatball menu HTML
        let meatballMenuHTML = '<span style="color: #999;">—</span>';
        if (order.id && menuItems.length > 0) {
            const escapedOrderIdForToggle = String(order.id).replace(/'/g, "\\'").replace(/"/g, '\\"');
            meatballMenuHTML = `
                <div class="order-meatball-menu">
                    <button class="meatball-menu-btn" type="button" onclick="toggleOrderMeatballMenu('${escapedOrderIdForToggle}', event)" aria-label="Order actions">
                        <i class="fas fa-ellipsis-v"></i>
                    </button>
                    <div class="meatball-menu-dropdown" id="orderMeatballMenu_${order.id}" style="display: none;">
                        ${menuItems.join('')}
                    </div>
                </div>
            `;
        ...
        row.innerHTML = `
            ...
            <td class="address-column">${location}</td>
...
function formatOrderCustomer(order) {
    ...
    // Return the userId as the user ID
...
function toggleMenuTabDropdown() {
    ...
    localStorage.setItem('menuTabDropdownOpen', willShow ? 'true' : 'false');
}
...
function toggleCustomerTabDropdown() {
    ...
    // Save state to localStorage
    localStorage.setItem('customerTabDropdownOpen', willShow ? 'true' : 'false');
}
...
function togglePromotionTabDropdown() {
    ...
    // Save state to localStorage
    localStorage.setItem('promotionTabDropdownOpen', willShow ? 'true' : 'false');
}
...
function toggleAnalyticsTabDropdown() {
    ...
    // Save state to localStorage
    localStorage.setItem('analyticsTabDropdownOpen', willShow ? 'true' : 'false');
}

// Restore dropdown states from localStorage on page load
function restoreDropdownStates() {
    const dropdowns = [
        { id: 'menuTabDropdown', key: 'menuTabDropdownOpen' },
        { id: 'customerTabDropdown', key: 'customerTabDropdownOpen' },
        { id: 'promotionTabDropdown', key: 'promotionTabDropdownOpen' },
        { id: 'analyticsTabDropdown', key: 'analyticsTabDropdownOpen' }
    ];
...
// Initialize dropdown states when DOM is ready
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', restoreDropdownStates);
} else {
...
function applyOrderFilters() {
    ...
    // Build the filter value based on selected options
    let filterValue = sortFilter ? sortFilter.value : 'new-to-old';
    
    const statusValue = statusFilter ? statusFilter.value : '';
    const serviceTypeValue = serviceTypeFilter ? serviceTypeFilter.value : '';
    
    // Store individual filter values for the new filtering system
    orderFilters.sort = filterValue;
    orderFilters.status = statusValue;
    orderFilters.serviceType = serviceTypeValue;
    
    // For backward compatibility with old filter system
    if (statusValue && serviceTypeValue) {
        orderFilters.filter = `type:${serviceTypeValue}|status:${statusValue}`;
    } else if (statusValue) {
        orderFilters.filter = `status:${statusValue}`;
    } else if (serviceTypeValue) {
        orderFilters.filter = `type:${serviceTypeValue}`;
    } else {
...
async function updateOrderStatus(orderId, newStatus) {
    ...
    if (normalizedNewStatus === 'preparing') {
        ...
        // Can move to completed from ready (dine-in), ready for pick-up (pick-up), or out for delivery (delivery)
        ...
        errorMessage = 'Order must be "Ready" (dine-in), "Ready for Pick-up" (pick-up), or "Out for Delivery" (delivery) before marking as completed.';
    } else {
...
async function confirmDeclinePayment() {
    ...
    try {
        ...
        showNotification('Failed to decline order. Please try again.', 'error');
    }
}
...
// Update the stock status document for customer-side tracking
async function updateStockStatusDocument() {
    if (!isFirestoreReady()) {
        await waitForFirebaseReady();
    }
    
    if (!menuState || !menuState.length) {
        console.log('[Stock Status] No menu items to check');
        return;
    }
    
    const fns = window.firestoreFunctions;
    if (!fns) {
        console.warn('[Stock Status] Firestore functions not available');
        return;
    }
    ...
    try {
        ...
        // Check each active menu item for out-of-stock status
        for (const item of menuState) {
            // Only check active items (inactive items won't show to customers anyway)
            ...
            if (hasVariations) {
                // For items with variations: out of stock if ALL variations are out of stock
                const allVariationsOutOfStock = item.variations.every(variation => {
                    const varQty = getMenuQuantity(item, variation);
                    return varQty <= 0;
                });
                isOutOfStock = allVariationsOutOfStock;
            } else {
                // For items without variations: check the item's quantity
                const itemQty = getMenuQuantity(item);
                isOutOfStock = itemQty <= 0;
            }
        ...
        // Update the menuStocks/stockStatus document
        const stockStatusRef = fns.doc(window.db, 'menuStocks', 'stockStatus');
        await fns.setDoc(stockStatusRef, {
            outOfStockItems: outOfStockItems,
            updatedAt: fns.serverTimestamp()
        }, { merge: true });
        
        console.log(`[Stock Status] Updated stock status document. Out of stock items: ${outOfStockItems.length}`, outOfStockItems);
    } catch (error) {
        console.error('[Stock Status] Failed to update stock status document:', error);
        // Don't throw - this is a non-critical operation
    }
}

// Manual trigger for testing - can be called from console: updateStockStatus()
window.updateStockStatus = updateStockStatusDocument;
...
async function refreshMenuState() {
    ...
    // Check for expired archived items (older than 30 days) - no longer auto-deletes
    try {
        const expiredCount = await MenuStore.cleanupExpiredArchivedItems();
        // If there are items pending permanent deletion, notify the user
        if (expiredCount > 0 && window.pendingPermanentDeletions && window.pendingPermanentDeletions.length > 0) {
            // Show notification about pending deletions - user must manually confirm
            setTimeout(() => {
                showPendingDeletionsNotification(window.pendingPermanentDeletions);
            }, 2000); // Delay to let page load first
        }
    } catch (error) {
        console.error('Failed to check expired archived items:', error);
        // Don't block menu loading if check fails
    }
    ...
    // Update stock status document for customer-side tracking
    try {
        await updateStockStatusDocument();
    } catch (error) {
        console.error('Failed to update stock status document:', error);
        // Don't block menu loading if stock status update fails
    }
...
// Search filter for menu list
let menuListSearchQuery = '';

function filterMenuListBySearch() {
    const searchInput = document.getElementById('menuListSearch');
    if (searchInput) {
        menuListSearchQuery = searchInput.value.toLowerCase().trim();
        renderMenuListTable();
    }
}
...
function applyMenuListFilter(filterType, filterValue) {
    ...
    const dropdown = document.getElementById('menuListCategoryDropdown');
    if (dropdown) {
...
// Apply stock status filter to menu list
function applyMenuListStockFilter(filterValue) {
    currentMenuListStockFilter = filterValue;
    renderMenuListTable();
    // Update active state in dropdown
    const dropdown = document.getElementById('menuListStockDropdown');
    if (dropdown) {
        dropdown.querySelectorAll('.dropdown-item').forEach(item => {
            item.classList.remove('active');
            if (item.getAttribute('data-filter-value') === filterValue) {
                item.classList.add('active');
            }
        });
    }
...
window.applyMenuListStockFilter = applyMenuListStockFilter;
...
function filterMenuListItems(items) {
    ...
    if (menuListSearchQuery && menuListSearchQuery.trim() !== '') {
        const query = menuListSearchQuery.toLowerCase().trim();
        filtered = filtered.filter(item => {
            ...
            const hasVariations = Array.isArray(item.variations) && item.variations.length > 0;
            const variationMatch = hasVariations && item.variations.some(v => {
                const vName = (v.name || '').toLowerCase();
                return vName.includes(query);
            });
            
            return name.includes(query) || 
                   menuId.includes(query) || 
    ...
    if (currentMenuListFilter.status === 'active') {
        filtered = filtered.filter(item => {
            // Items with 2+ variations should always show (they're parent items that need to be visible)
            const hasMultipleVariations = Array.isArray(item.variations) && item.variations.length >= 2;
    ...
    if (currentMenuListStockFilter !== 'all') {
        filtered = filtered.filter(item => {
            const hasVariations = Array.isArray(item.variations) && item.variations.length > 0;
            let isOutOfStock = false;
            ...
            if (currentMenuListStockFilter === 'in-stock') {
                return !isOutOfStock;
            } else if (currentMenuListStockFilter === 'out-of-stock') {
                return isOutOfStock;
            }
            return true;
        });
...
async function updateMenuQuantity(itemId, newQuantity) {
    ...
    try {
        ...
        // Check both menu and menuArchives collections (item might be inactive/archived)
        const menuRef = fns.doc(window.db, 'menu', itemId);
        const archiveRef = fns.doc(window.db, 'menuArchives', itemId);
        ...
        if (menuDoc.exists()) {
            itemRef = menuRef;
            itemData = menuDoc.data();
        } else if (archiveDoc.exists()) {
            itemRef = archiveRef;
            itemData = archiveDoc.data();
            isInArchive = true;
        } else {
        ...
        // If item is inactive and new quantity > 0, offer to reactivate it
        const wasInactive = itemData.isActive === false || isInArchive;
        if (wasInactive && quantity > 0) {
            // Auto-reactivate the item
            try {
                await MenuStore.updateItem(itemId, { isActive: true });
                showNotification(`Quantity updated and "${itemData.name || itemData.displayName || 'item'}" has been reactivated.`, 'success');
            } catch (reactivateError) {
                console.error('Error reactivating item:', reactivateError);
                showNotification('Quantity updated, but failed to reactivate item. Please reactivate manually.', 'warning');
            }
        } else {
...
async function updateMenuVariationQuantity(itemId, variationId, newQuantity) {
    ...
    try {
        ...
        // If item is inactive and new quantity > 0, check if we should reactivate
        const wasInactive = itemData.isActive === false || isInArchive;
        if (wasInactive && quantity > 0) {
            // Check if at least one variation now has stock
            const updatedVariations = variations;
            const hasAnyStock = updatedVariations.some(v => {
                const varQty = v.quantity !== undefined && v.quantity !== null ? Number(v.quantity) : 0;
                return varQty > 0;
            });
            ...
            if (hasAnyStock) {
                ...
                try {
                    await MenuStore.updateItem(itemId, { isActive: true });
                    showNotification(`Variation quantity updated and "${itemData.name || itemData.displayName || 'item'}" has been reactivated.`, 'success');
                } catch (reactivateError) {
                    console.error('Error reactivating item:', reactivateError);
                    showNotification('Variation quantity updated, but failed to reactivate item. Please reactivate manually.', 'warning');
                }
            } else {
...
function updateMenuListCategoryFilter() {
    const categoryDropdown = document.getElementById('menuListCategoryDropdown');
    if (!categoryDropdown || !menuState || !menuState.length) return;
    ...
    allLink.setAttribute('data-filter-type', 'category');
    allLink.setAttribute('data-filter-value', 'all');
    allLink.textContent = 'All Categories';
    allLink.onclick = function() {
        applyMenuListFilter('category', 'all');
        return false;
    };
...
let currentMenuListStockFilter = 'all'; // 'all', 'in-stock', 'out-of-stock'
...
function renderMenuDetailsCarousel() {
    ...
    if (imageContainer) {
        ...
        if (item.imageDataUrl && item.imageDataUrl.trim() !== '') {
            // Set image directly via innerHTML - no caption overlay (name is in header now)
            imageContainer.innerHTML = `
                <img id="menuDetailImage" src="${item.imageDataUrl}" alt="${displayName || 'Menu item image'}" style="width: 100%; height: 100%; object-fit: cover; display: block;">
            `;
    ...
    // Menu name is now in the header, update it there
    const menuNameEl = document.getElementById('menuDetailName');
    if (menuNameEl) menuNameEl.textContent = item.displayName || item.name || 'MENU ITEM';
    ...
    reader.onload = function(e) {
        ...
        if (imageContainer) {
            ...
            if (imageEl) {
                ...
                imageContainer.innerHTML = `
                    <img id="menuDetailImage" src="${e.target.result}" alt="${itemName}" style="width: 100%; height: 100%; object-fit: cover; display: block;">
                `;
            }
        }
    };
...
// Show notification about menu items pending permanent deletion
function showPendingDeletionsNotification(pendingItems) {
    if (!pendingItems || pendingItems.length === 0) return;
    
    const itemNames = pendingItems.map(item => item.name).join(', ');
    const itemCount = pendingItems.length;
    
    const dialog = document.createElement('div');
    dialog.className = 'modal';
    dialog.id = 'pendingDeletionsModal';
    ...
    dialog.innerHTML = `
        <div class="modal-content" style="max-width: 600px; width: 90%; background: white; border-radius: 8px; box-shadow: 0 4px 20px rgba(0,0,0,0.15); animation: slideDown 0.3s ease-out;">
            <div class="modal-header" style="padding: 20px 24px; border-bottom: 1px solid #e9ecef; display: flex; align-items: center; gap: 12px;">
                <div style="width: 44px; height: 44px; border-radius: 50%; background: #fff3cd; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                    <i class="fas fa-exclamation-triangle" style="color: #856404; font-size: 22px;"></i>
                </div>
                <div>
                    <h2 style="margin: 0; color: #212529; font-size: 20px; font-weight: 600;">Items Pending Permanent Deletion</h2>
                    <p style="margin: 4px 0 0 0; color: #6c757d; font-size: 14px;">${itemCount} menu item${itemCount > 1 ? 's' : ''} in archive for over 30 days</p>
                </div>
            </div>
            <div class="modal-body" style="padding: 24px;">
                <p style="margin: 0 0 16px 0; color: #495057; font-size: 15px; line-height: 1.5;">
                    The following menu items have been in the archive for more than 30 days and are pending permanent deletion:
                </p>
                <div style="background: #f8f9fa; border: 1px solid #e9ecef; border-radius: 8px; padding: 16px; max-height: 200px; overflow-y: auto;">
                    <ul style="margin: 0; padding-left: 20px; color: #495057;">
                        ${pendingItems.map(item => `<li style="margin-bottom: 8px;"><strong>${item.name}</strong> <span style="color: #6c757d; font-size: 13px;">(deleted ${Math.floor((new Date() - item.deletedAt) / (1000 * 60 * 60 * 24))} days ago)</span></li>`).join('')}
                    </ul>
                </div>
                <div style="background: #d4edda; border: 1px solid #c3e6cb; border-radius: 8px; padding: 16px; margin-top: 16px;">
                    <p style="margin: 0; color: #155724; font-size: 14px;">
                        <i class="fas fa-info-circle" style="margin-right: 8px;"></i>
                        <strong>No automatic deletion:</strong> These items will NOT be deleted automatically. You must confirm to permanently delete them, or you can restore them from the archive.
                    </p>
                </div>
            </div>
            <div class="modal-footer" style="padding: 16px 24px; border-top: 1px solid #e9ecef; display: flex; gap: 12px; justify-content: flex-end;">
                <button type="button" id="dismissPendingDeletionsBtn" style="padding: 10px 20px; border: 1px solid #6c757d; background: white; color: #495057; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 500;">
                    Dismiss
                </button>
                <button type="button" id="confirmPermanentDeleteBtn" style="padding: 10px 20px; border: none; background: #dc3545; color: white; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 500;">
    ...
    // Dismiss button
    document.getElementById('dismissPendingDeletionsBtn').addEventListener('click', () => {
        dialog.remove();
    });
    ...
    document.getElementById('confirmPermanentDeleteBtn').addEventListener('click', async () => {
        ...
        try {
            await MenuStore.permanentlyDeleteExpiredItems(pendingItems);
            showNotification(`Permanently deleted ${itemCount} menu item${itemCount > 1 ? 's' : ''}.`, 'success');
            dialog.remove();
            // Refresh menu state
            menuState = await MenuStore.getItems();
            renderMenuState();
            ...
            confirmBtn.innerHTML = '<i class="fas fa-trash" style="margin-right: 6px;"></i> Permanently Delete All';
        }
    });
...
// Make function available globally
window.showPendingDeletionsNotification = showPendingDeletionsNotification;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    } else if (currentPage === 'orders.html' || window.location.pathname.includes('orders.html')) {
...
window.restoreDropdownStates = restoreDropdownStates;
...
async function backfillForDeliveryDocuments() {
    try {
        ...
        // Get all orders that have a driver assigned and are out for delivery or completed
        const ordersWithDrivers = ordersState.filter(order => {
            const hasDriver = order.driverId && order.driverId.trim() !== '';
            const isOutForDeliveryOrCompleted = order.status === 'out_for_delivery' || 
                                    order.status === 'out-for-delivery' || 
                                    order.status === 'in-transit' ||
                                    order.status === 'in_transit' ||
                                    order.status === 'completed' ||
                                    order.status === 'delivered';
            return hasDriver && isOutForDeliveryOrCompleted;
        });
            ...
            try {
                ...
                if (!existingDoc.exists()) {
                    ...
                    await fns.setDoc(deliveryRef, {
                        ...
                        timeCompleted: isOrderCompleted ? (order.completedAt || order.deliveredAt || fns.serverTimestamp()) : null,
                    ...
                    // Update if driver changed or timeCompleted is missing
                    const existingData = existingDoc.data();
                    const isOrderCompleted = order.status === 'completed' || order.status === 'delivered';
                    const needsUpdate = existingData.driverId !== order.driverId || 
                                       (isOrderCompleted && !existingData.timeCompleted && !existingData.timeDelivered);
                    ...
                    if (needsUpdate) {
                        ...
                        if (isOrderCompleted && !existingData.timeCompleted && !existingData.timeDelivered) {
                            updateData.timeCompleted = order.completedAt || order.deliveredAt || fns.serverTimestamp();
                        }
...
window.filterMenuListBySearch = filterMenuListBySearch;
...
// Store banners data for filtering
let allBanners = [];
...
async function renderBannerCatalogue() {
    ...
    const tableEl = document.getElementById('bannersTable');
    const tbodyEl = document.getElementById('bannersTableBody');
    
    if (!loadingEl || !emptyEl || !tableEl || !tbodyEl) return;
    ...
    try {
        ...
        loadingEl.style.display = 'none';
        tbodyEl.innerHTML = `<tr><td colspan="7" style="text-align: center; padding: 40px; color: #dc3545;">
            <i class="fas fa-exclamation-triangle" style="font-size: 2em; margin-bottom: 16px;"></i>
...
// Display banners in table format
function displayBanners(banners) {
    ...
    banners.forEach(banner => {
        ...
        const statusClass = isActive ? 'status-completed' : 'status-cancelled';
        const statusText = isActive ? 'Active' : 'Inactive';
        
        row.innerHTML = `
            <td>
                <div style="width: 80px; height: 50px; border-radius: 4px; overflow: hidden; background: #f5f5f5;">
                    <img src="${banner.imageUrl || ''}" alt="${escapeHtml(banner.title || 'Banner')}" 
                         style="width: 100%; height: 100%; object-fit: cover; cursor: pointer;" 
                         onclick="viewBannerImage('${banner.imageUrl || ''}', '${escapeHtml(banner.title || 'Banner')}')">
                </div>
            </td>
            <td><strong>${escapeHtml(banner.title || 'Untitled Banner')}</strong></td>
            <td style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${escapeHtml(banner.description || '—')}</td>
            <td>${escapeHtml((banner.placement || 'banner').toUpperCase())}</td>
            ...
            <td>
                <div class="action-buttons" style="display: flex; gap: 8px;">
                    <button class="btn btn-sm" style="background-color: #3d2817; color: #f6c056; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer;" onclick="editBanner('${banner.id}')">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="btn btn-sm btn-danger" style="padding: 6px 12px; border-radius: 4px; cursor: pointer;" onclick="deleteBanner('${banner.id}', '${escapeHtml(banner.title || 'Banner')}')">
...
function filterBanners() {
    ...
    const placementValue = placementFilter?.value || '';
    ...
    if (statusValue) {
        ...
        filtered = filtered.filter(banner => {
            ...
            const endDate = banner.endDate?.toDate?.() || null;
            const isActive = startDate && endDate && now >= startDate && now <= endDate;
            return statusValue === 'active' ? isActive : !isActive;
        });
    }
    
    // Filter by placement
    if (placementValue) {
        filtered = filtered.filter(banner => (banner.placement || 'banner').toLowerCase() === placementValue.toLowerCase());
    }
    
    displayBanners(filtered);
}

// View banner image in full resolution
function viewBannerImage(imageUrl, title) {
    if (!imageUrl) {
    ...
    modal.innerHTML = `
        <div style="position: relative; max-width: 95%; max-height: 95%;">
            <button onclick="this.parentElement.parentElement.remove()" style="position: absolute; top: -40px; right: 0; background: none; border: none; color: white; font-size: 30px; cursor: pointer;">&times;</button>
            <img src="${imageUrl}" alt="${escapeHtml(title)}" style="max-width: 100%; max-height: 90vh; object-fit: contain; border-radius: 8px;">
            <div style="text-align: center; color: white; margin-top: 12px; font-size: 16px;">${escapeHtml(title)}</div>
        </div>
    `;
...
// Edit banner - open modal with full image and editable fields
async function editBanner(bannerId) {
    ...
    document.getElementById('editBannerCurrentImageUrl').value = banner.imageUrl || '';
    document.getElementById('editBannerImage').src = banner.imageUrl || '';
    ...
    document.getElementById('editBannerPlacement').value = banner.placement || 'banner';
    ...
    if (startDate) {
        document.getElementById('editBannerStartDate').value = formatDateTimeLocal(startDate);
    }
    if (endDate) {
        document.getElementById('editBannerEndDate').value = formatDateTimeLocal(endDate);
    }
    
    // Clear file input
    document.getElementById('editBannerNewImage').value = '';
...
function previewEditBannerImage(event) {
    ...
    if (file) {
        const reader = new FileReader();
        reader.onload = function(e) {
            document.getElementById('editBannerImage').src = e.target.result;
        };
        reader.readAsDataURL(file);
    }
}

// Close edit banner modal
...
// Handle edit banner form submit
async function handleEditBannerSubmit(event) {
    event.preventDefault();
    ...
    const placement = document.getElementById('editBannerPlacement').value;
    const startDateStr = document.getElementById('editBannerStartDate').value;
    const endDateStr = document.getElementById('editBannerEndDate').value;
    const newImageFile = document.getElementById('editBannerNewImage').files[0];
    let imageUrl = document.getElementById('editBannerCurrentImageUrl').value;
    ...
    try {
        ...
        if (newImageFile) {
            // Convert to base64 for storage (or use Firebase Storage if available)
            imageUrl = await convertFileToBase64(newImageFile);
        }
        ...
        const updateData = {
            ...
            description: description,
            placement: placement,
            startDate: fns.Timestamp.fromDate(startDate),
            endDate: fns.Timestamp.fromDate(endDate),
            imageUrl: imageUrl,
            updatedAt: fns.Timestamp.fromDate(new Date())
        };
...
// Store table numbers data for filtering
let allTableNumbers = [];
...
async function renderTableNumbers() {
    ...
    const tableEl = document.getElementById('tableNumbersTable');
    const tbodyEl = document.getElementById('tableNumbersTableBody');
    ...
    try {
        ...
        // Render the table
        displayTableNumbers(allTableNumbers);
        
    } catch (error) {
        console.error('Error loading table numbers:', error);
        loadingEl.style.display = 'none';
        tbodyEl.innerHTML = `<tr><td colspan="6" style="text-align: center; padding: 40px; color: #dc3545;">
            <i class="fas fa-exclamation-triangle" style="font-size: 2em; margin-bottom: 16px;"></i>
...
// Display table numbers in table format
function displayTableNumbers(tableNumbers) {
    ...
    tableNumbers.forEach(table => {
        ...
        const statusClass = table.isActive !== false ? 'status-completed' : 'status-cancelled';
        const statusText = table.isActive !== false ? 'Active' : 'Inactive';
        
        row.innerHTML = `
            <td>
                <div style="display: flex; align-items: center; gap: 8px;">
                    <i class="fas fa-table" style="color: #7E2021;"></i>
                    <strong>${escapeHtml(table.name || 'Unnamed Table')}</strong>
                </div>
            </td>
            <td>
                <code style="font-size: 12px; background: #f8f9fa; padding: 4px 8px; border-radius: 4px; word-break: break-all;">
            ...
            <td>
                <div class="action-buttons" style="display: flex; gap: 8px;">
                    <button class="btn btn-sm" style="background-color: #3d2817; color: #f6c056; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer;" onclick="editTableNumber('${table.id}')">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="btn btn-sm btn-danger" style="padding: 6px 12px; border-radius: 4px; cursor: pointer;" onclick="deleteTableNumber('${table.id}', '${escapeHtml(table.name || 'Table')}')">
                        <i class="fas fa-trash"></i> Delete
                    </button>
                </div>
            </td>
...
// Filter table numbers based on search and status
function filterTableNumbers() {
    const searchInput = document.getElementById('tableNumberSearch');
    const statusFilter = document.getElementById('tableNumberStatusFilter');
    
    const searchTerm = (searchInput?.value || '').toLowerCase().trim();
    const statusValue = statusFilter?.value || '';
    
    let filtered = [...allTableNumbers];
    ...
    if (statusValue === 'active') {
        filtered = filtered.filter(table => table.isActive !== false);
    } else if (statusValue === 'inactive') {
        filtered = filtered.filter(table => table.isActive === false);
    }
    
    displayTableNumbers(filtered);
}
...
window.displayBanners = displayBanners;
window.filterBanners = filterBanners;
window.viewBannerImage = viewBannerImage;
window.editBanner = editBanner;
window.closeEditBannerModal = closeEditBannerModal;
window.handleEditBannerSubmit = handleEditBannerSubmit;
...
window.displayTableNumbers = displayTableNumbers;
window.filterTableNumbers = filterTableNumbers;
...
function showUserProfile() {
    // Get the current logged-in user's staff ID from session
    ...
    if (session) {
        try {
            ...
            if (staffId) {
                ...
                window.location.href = 'view-user-profile.html?id=' + encodeURIComponent(staffId);
                return;
            }
        } catch (e) {
    ...
    // Fallback: if no session or staffId, go to admin profile page
        ...
        if (deliveryCard) {
            ...
            if (actionsElement) {
                actionsElement.innerHTML = `
                    <button class="btn btn-warning" onclick="markCompleted('${orderId}')">
...
async function markCompleted(orderId) {
    if (confirm(`Mark order #${orderId} as completed?`)) {
        try {
            ...
            await fns.updateDoc(orderRef, {
                status: 'completed',
                completedAt: fns.serverTimestamp(),
                updatedAt: fns.serverTimestamp()
            });
            
            // Update for_delivery document with time completed (for delivery orders)
            const deliveryRef = fns.doc(window.db, 'for_delivery', orderId);
            const deliveryDoc = await fns.getDoc(deliveryRef);
            
            if (deliveryDoc.exists()) {
                await fns.updateDoc(deliveryRef, {
                    timeCompleted: fns.serverTimestamp(),
                    updatedAt: fns.serverTimestamp()
                });
            } else {
            ...
            const orderIndex = ordersState.findIndex(o => o.id === orderId);
            if (orderIndex !== -1) {
                ordersState[orderIndex].status = 'completed';
            }
            ...
            const deliveryCard = document.querySelector(`[onclick*="markCompleted('${orderId}')"]`)?.closest('.delivery-card');
        if (deliveryCard) {
            const statusElement = deliveryCard.querySelector('.delivery-status');
            const actionsElement = deliveryCard.querySelector('.delivery-actions');
            ...
            if (statusElement) {
                ...
                statusElement.className = 'delivery-status completed';
            }
        ...
        showNotification(`Order #${orderId} marked as completed!`, 'success');
            ...
            showNotification('Failed to mark order as completed. Please try again.', 'error');
        }
    }
}
...
async function getActiveDeliveries() {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            const timeCompleted = deliveryData.timeCompleted || deliveryData.timeDelivered;
            const driverId = deliveryData.driverId || '';
            
            // Active delivery = has driverId and timeCompleted is null/undefined
            if (driverId && (timeCompleted === null || timeCompleted === undefined)) {
...
async function updateDriverStatusesWithOrders() {
    ...
    // Get active deliveries from for_delivery collection (where timeCompleted is null)
